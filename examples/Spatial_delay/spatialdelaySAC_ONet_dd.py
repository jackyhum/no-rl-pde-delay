...
from pde_control_gym.src import SpatialDelayReward2
import math
...
def resetTau(val):
    ampT = 1 # np.random.uniform(-0.4, 0.)
    gammaT = 0.8 # np.random.uniform(3, 5)
...
def main():
    ...
    spatialDelayParameters = {
            ...
            "c1": c1,
            "spatial": spatial,
            "reward_class": SpatialDelayReward2(int(round(T/dt)), N1, 2 , -1e3, 200, 10),
            "normalize":True,  
    ...
    model = SAC("MlpPolicy",
                ...
                learning_rate = 9e-5,#learning_rate_schedule,
                batch_size = 512,
                train_freq = 2,
                tau=0.003,
    ...
    if retrain:
        model.learn(total_timesteps = total_steps, 
                    tb_log_name="ronet_dd",
                    # callback = eval_callback,
                )
        model.save("./PDEControlGym/Model/ronet_dd.zip")
        print("*---------训练结束-----------*")
    ...
    env = gym.make("PDEControlGym-SpatialDelayPDE", **spatialDelayParameters)
    obs,__ = env.reset()
    model = SAC.load("./PDEControlGym/Model/ronet_dd.zip")
    uStorage.append(obs)
    ...
    end_time = time.time() 
    print("Time for NO-SAC:", end_time - start_time, "seconds")
    u = np.array(uStorage)
    ...
    plt.savefig('./PDEControlGym/pics/RONET_dd/test3.png', dpi=300, bbox_inches='tight', pad_inches=0.3)
    # plt.show()
    ...
    plt.savefig('./PDEControlGym/pics/RONET_dd/control3.png', dpi=300, bbox_inches='tight')
    ...
    plt.xlabel(r"$t(\text{sec})$",fontsize="13")
    plt.tick_params(labelsize=13)
    plt.savefig('./PDEControlGym/pics/RONET_dd/L2.png', dpi=300, bbox_inches='tight')
    np.savez("./PDEControlGym/pics/DataForPics/ronet_data_dd.npz", time = time2, ronet_control = u[:, N1 ], ronet_l2 = interL2)
    print("end")
    ...
    # env = gym.make("PDEControlGym-SpatialDelayPDE", **spatialDelayParameters)
    # obs,__ = env.reset()
    # model = SAC.load("./PDEControlGym/Model/spatial_delay_model.zip")
    # uStorage.append(obs)
    ...
    # plt.savefig('./PDEControlGym/pics/RONET/noise/test3.png', dpi=300)
    # # plt.show()
    ...
    # plt.tick_params(labelsize=13)
    # plt.savefig('./PDEControlGym/pics/RONET/noise/control3.png', dpi=300)
    ...
    # plt.xlabel(r'$Time$',fontsize="13")
    # plt.tick_params(labelsize=13)
    # plt.savefig('./PDEControlGym/pics/RONET/noise/L2.png', dpi=300)
