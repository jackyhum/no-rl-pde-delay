...
def main():
    ...
    spatialDelayParameters = {
            ...
            "c1": c1,
            "spatial": spatial,
            "reward_class": SpatialDelayReward2(int(round(T/dt)), N1, 2 , -9e5, 300, 10),
            "normalize":True,  
    ...
    if retrain:
        model.learn(total_timesteps = total_steps, 
                    tb_log_name="onenet_dd",
                    # callback = eval_callback,
                )
        model.save("./PDEControlGym/Model/branchnet_model_dd.zip")
        print("*---------训练结束-----------*")
    ...
    env = gym.make("PDEControlGym-SpatialDelayPDE", **spatialDelayParameters)
    obs,__ = env.reset()
    model = SAC.load("./PDEControlGym/Model/branchnet_model_dd.zip")
    uStorage.append(obs)
    ...
    end_time = time.time()
    print("Time for baseline SAC", end_time - start_time, "seconds")
    u = np.array(uStorage)
    ...
    plt.savefig('./PDEControlGym/pics/BranchNet_dd/test3.png', dpi=300)
    ...
    plt.savefig('./PDEControlGym/pics/BranchNet_dd/control3.png', dpi=300, bbox_inches='tight')
    ...
    plt.xlabel(r"$t(\text{sec})$",fontsize="13")
    plt.tick_params(labelsize=13)
    plt.savefig('./PDEControlGym/pics/BranchNet_dd/L2.png', dpi=300, bbox_inches='tight')
    
    np.savez("./PDEControlGym/pics/DataForPics/branch_data_dd.npz", time = time2, branch_control = u[:, N1 ], branch_l2 = interL2)
    print("end")
